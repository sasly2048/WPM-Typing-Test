...
import { getSessions } from '../services/history.js';
...
export function render(container) {
  ...
  const bestKey = `pb-${session.mode || 'default'}`;
  const previousBest = Number(localStorage.getItem(bestKey)) || 0;
  const isPersonalBest = netWpm > previousBest;
  if (isPersonalBest) localStorage.setItem(bestKey, netWpm);

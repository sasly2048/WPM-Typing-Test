document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchNewText() {
        ...
        try {
            ...
            textDisplay.textContent = "Failed to load paragraph. Please ensure PDF is processed.";
            userInput.disabled = true;
            startButton.disabled = true;
        }
    }
    ...
    function calculateWPM() {
        ...
        const words = correctTypedSection.split(/\s+/).filter(word => word.length > 0).length;
    ...
    function updateTimerDisplay() {
        ...
        timeLeftSpan.textContent = `${minutes}m ${seconds.toString().padStart(2, '0')}s`;
    }
    ...
    resetButton.addEventListener('click', () => {
        if (confirm("Are you sure you want to reset the current test? This will load a new paragraph.")) {
            resetTestState();
            fetchNewText();
        }
    });
